...
export function Sidebar() {
  // Esta função aplica o fundo azul automaticamente se estivermos na página correta
  const getLinkStyle = ({ isActive }: { isActive: boolean }) => 
    `flex items-center gap-3 px-3 py-2 rounded-md text-sm font-medium transition-colors ${
      isActive 
        ? 'bg-[#0a1526] text-white' // Cor escura para o item selecionado (baseado na sua imagem)
